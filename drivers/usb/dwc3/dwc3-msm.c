...
static void dwc3_adc_notification(enum qpnp_tm_state state, void *ctx)
{
	...
	qpnp_vadc_read(mdwc->vadc_id_dev,P_MUX1_1_1, &results);
	dev_err(mdwc->dev, "usb id adc = %d\n", (int)results.physical);
...
skip_id_work:
	...
	qpnp_vadc_channel_monitor(mdwc->vadc_id_dev, &mdwc->adc_param);
}
...
static void dwc3_init_adc_work(struct work_struct *w)
{
		...
		mdwc->vadc_id_dev = qpnp_get_vadc(mdwc->dev, "dwc");
		 if (IS_ERR(mdwc->vadc_id_dev)) {
			 if (PTR_ERR(mdwc->vadc_id_dev) == -EPROBE_DEFER) {
				 dev_err(mdwc->dev, "%s: qpnp vadc not yet "
					"probed.\n",  __func__);
				 queue_delayed_work(mdwc->sm_usb_wq, to_delayed_work(w),
	...
	mdwc->adc_param.channel = P_MUX1_1_1;

	ret = qpnp_vadc_channel_monitor(mdwc->vadc_id_dev, &mdwc->adc_param);
	if (ret) {
