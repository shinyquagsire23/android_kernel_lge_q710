...
/* Added for camera hardware info mh4 2018-09-14 begin */
#ifdef CONFIG_GET_HARDWARE_INFO
#include <linux/hardware_info.h>
#endif
/* Added for camera hardware info mh4 2018-09-14 end */
...
/* Added for camera hardware info mh4 2018-09-14 begin */
	...
	if(slave_info->sensor_init_params.position == BACK_CAMERA_B){
		if(slave_info->sensor_module_info){
			register_hardware_info(MAIN_CAM, slave_info->sensor_name);
		}else{
			pr_err("%s hardware info is NULL.\n",slave_info->sensor_name);
		}
	}else if(slave_info->sensor_init_params.position == FRONT_CAMERA_B){
		if(slave_info->sensor_module_info){
			register_hardware_info(SUB_CAM, slave_info->sensor_name);
		}else{
			pr_err("%s hardware info is NULL.\n",slave_info->sensor_name);
		}
	}else if(slave_info->sensor_init_params.position == AUX_CAMERA_B){
		if(slave_info->sensor_module_info){
			register_hardware_info(AUX_CAM, slave_info->sensor_name);
		}else{
			pr_err("%s hardware info is NULL.\n",slave_info->sensor_name);
		}
	}else{
