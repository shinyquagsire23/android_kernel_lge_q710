...
#if defined(CONFIG_MACH_MSM8917_CV1_LAO_COM) || defined(CONFIG_MACH_MSM8917_CV3_LAO_COM) || defined(CONFIG_MACH_MSM8917_CV1S_LAO_COM)
...
bool otg_en_boost;
#endif
#if defined(CONFIG_LGE_USB_TYPE_C) || defined(CONFIG_LGE_PM_CABLE_DETECTION) || defined(CONFIG_LGE_PM_LGE_POWER_CLASS_CABLE_DETECT)
...
#else
#if defined(CONFIG_LGE_PM_CABLE_DETECTION) || defined(CONFIG_LGE_PM_LGE_POWER_CLASS_CABLE_DETECT)
	/* Disable PMI8952 USB ID pin */
	rc = smbchg_sec_masked_write(chip, chip->otg_base + OTG_CFG_REG, OTG_CTRL_MASK, 0x0);
	if (rc < 0)
		dev_err(chip->dev, "Couldn't set OTG EN config rc = %d\n", rc);
...
//START for CCD daemon
#ifdef CONFIG_LGE_PM_CCD
static int smbchg_ccd_set_fcc(int val)
{
	int ret;
	...
	the_chip->ccd_fcc = val;
	return 0;
}

static int smbchg_ccd_set_icl(int val)
{
	int ret;
	...
	the_chip->ccd_icl = val;
	return 0;
}

static int smbchg_ccd_set_batchg_en(int val)
{
	int ret;
	...
	the_chip->ccd_batchg_en = smbchg_ccd_batchg_en;
	return 0;
}


static int smbchg_ccd_set_input_en(int val)
{
	int ret;
	...
	the_chip->ccd_input_en = smbchg_ccd_input_en;
	return 0;
}
...
static int get_prop_ccd_fcc(void)
{
	int val;
	val = the_chip->ccd_fcc;
	if (val  < 0) {
...
static int get_prop_ccd_icl(void)
{
	int val;
	val = the_chip->ccd_icl;
	if (val  < 0) {
		pr_smb(PR_STATUS, "Couldn't get ccd icl = %d\n", val);
		return -1;
	}
	return val;
}

static int get_prop_ccd_batchg_en(void)
{
	int val;
	val = the_chip->ccd_batchg_en;
	if (val  < 0) {
		pr_smb(PR_STATUS, "Couldn't get ccd batchg_en = %d\n", val);
		return -1;
	}
	return val;
}

static int get_prop_ccd_input_en(void)
{
	int val;
	val = the_chip->ccd_input_en;
	if (val  < 0) {
		pr_smb(PR_STATUS, "Couldn't get ccd input_en = %d\n", val);
		return -1;
	}
	return val;
}

static int get_prop_ccd_vfloat(void)
{
	int val;
	val = the_chip->ccd_vfloat;
	if (val  < 0) {
		pr_smb(PR_STATUS, "Couldn't get ccd vfloat = %d\n", val);
		return -1;
	}
	return val;
}
...
//START for CCD daemon
	...
	case POWER_SUPPLY_PROP_CCD_FCC :
		smbchg_ccd_set_fcc(val->intval);
		break;
	case POWER_SUPPLY_PROP_CCD_ICL :
		smbchg_ccd_set_icl(val->intval);
		break;
	case POWER_SUPPLY_PROP_CCD_BATCHG_EN :
		smbchg_ccd_set_batchg_en(val->intval);
		break;
	case POWER_SUPPLY_PROP_CCD_INPUT_EN :
		smbchg_ccd_set_input_en(val->intval);
		break;
...
#ifdef CONFIG_LGE_PM_CCD
	case POWER_SUPPLY_PROP_CCD_FCC :
		val->intval = get_prop_ccd_fcc();
		break;
	case POWER_SUPPLY_PROP_CCD_ICL :
		val->intval = get_prop_ccd_icl();
		break;
	case POWER_SUPPLY_PROP_CCD_BATCHG_EN :
		val->intval = get_prop_ccd_batchg_en();
		break;
	case POWER_SUPPLY_PROP_CCD_INPUT_EN :
		val->intval = get_prop_ccd_input_en();
		break;
	case POWER_SUPPLY_PROP_CCD_VFLOAT :
		val->intval = get_prop_ccd_vfloat();
		break;
#endif
//END for CCD daemon
	default:
