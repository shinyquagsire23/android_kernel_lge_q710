 ...
 */
#include "touch_ft8736.h"
#include "touch_ft8736_prd.h"
...
#ifdef FT8736_ESD_SKIP_WHILE_TOUCH_ON
static int finger_cnt = 0;

bool ft8736_check_finger(void)
{
	return finger_cnt==0? false:true;
}
EXPORT_SYMBOL(ft8736_check_finger);
#endif

#if defined(CONFIG_LGE_MODULE_DETECT)
int ft8736_panel_type;
#endif /* CONFIG_LGE_MODULE_DETECT */

/* touch irq handle according to display suspend in mfts */
bool mfts_check_shutdown = false;

int ft8736_reg_read(struct device *dev, u16 addr, void *data, int size)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
...
int ft8736_reg_write(struct device *dev, u16 addr, void *data, int size)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
	struct touch_bus_msg msg;
...
int ft8736_cmd_read(struct device *dev, void *cmd_data, int cmd_len, void *read_buf, int read_len)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
	struct touch_bus_msg msg;
...
static int ft8736_reset_ctrl(struct device *dev, int ctrl)
{
	struct touch_core_data *ts = to_touch_core(dev);
//	struct ft8736_data *d = to_ft8736_data(dev);
	TOUCH_TRACE();
	...
	case SW_RESET:
		TOUCH_I("%s : SW Reset\n", __func__);
//		ft8736_cmd_write(dev, CMD_ENA);
//		ft8736_cmd_write(dev, CMD_RESET_LOW);
		touch_msleep(5);
//		ft8736_cmd_write(dev, CMD_RESET_HIGH);
//		ft8736_cmd_write(dev, CMD_DIS);
...
static int ft8736_power(struct device *dev, int ctrl)
{
	struct touch_core_data *ts = to_touch_core(dev);
//	struct ft8736_data *d = to_ft8736_data(dev);
	TOUCH_TRACE();
	...
	case POWER_HW_RESET:
		TOUCH_I("%s, HW Reset\n", __func__);
		ft8736_reset_ctrl(dev, HW_RESET);
	}

	return 0;
}
static void ft8736_get_swipe_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static void ft8736_get_tci_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static struct ft8736_ic_info *ext_ic_info;

int ft8736_chipid_info(void)
{
	int ret = 0;
...
int ft8736_ic_info(struct device *dev)
{
//	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
	int ret = 0;
...
//	u8 rdata = 0;
	...
	for (i = 0; i < 2; i++) {
		ret |= ft8736_reg_read(dev, FTS_REG_ID, (u8 *)&chip_id, 1);
		ret |= ft8736_reg_read(dev, FTS_REG_ID_LOW, (u8 *)&chip_id_low, 1);
		ret |= ft8736_reg_read(dev, FTS_REG_FW_VER, (u8 *)&fw_version, 1);
		ret |= ft8736_reg_read(dev, FTS_REG_FW_VER_MINOR, (u8 *)&fw_version_minor, 1);
		ret |= ft8736_reg_read(dev, FTS_REG_FW_VER_SUB_MINOR, (u8 *)&fw_version_sub_minor, 1);
		ret |= ft8736_reg_read(dev, FTS_REG_FW_VENDOR_ID, (u8 *)&fw_vendor_id, 1);
		ret |= ft8736_reg_read(dev, FTS_REG_LIB_VER_H, (u8 *)&lib_version_high, 1);
		ret |= ft8736_reg_read(dev, FTS_REG_LIB_VER_L, (u8 *)&lib_version_low, 1);
...
#endif
static int ft8736_swipe_active_area(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	u8 active_area[8] = {0, };
	u8 start_area[8] = {0, };
	...
	/* Set by ft8736_get_swipe_info func */
	...
	ret = ft8736_reg_write(dev, SWIPE_ACTIVE_AREA_X1_H_CTRL, active_area, sizeof(active_area));
	ret = ft8736_reg_write(dev, SWIPE_START_AREA_X1_H_CTRL, start_area, sizeof(start_area));

	return ret;
}

static int ft8736_swipe_command(struct device *dev, int cmd)
{
	struct touch_core_data *ts = to_touch_core(dev);
	u8 swipe_data = 0;
	int ret = 0;
	...
	switch (cmd) {
		case SWIPE_ENABLE_CMD:
			...
			ret = ft8736_reg_write(dev, GESTURE_ON_CTRL, &swipe_data, sizeof(swipe_data));
			break;
		...
		case SWIPE_ACTIVE_AREA_CMD:
			ret = ft8736_swipe_active_area(dev);
			break;
...
static int ft8736_swipe_control(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	u8 swipe_data = {0x0};
	u8 swipe_data_2[7] = {0x0, };
	int ret = 0;
	int i = 0;
	...
	if (ts->lpwg.screen) { // Perhaps the part that needs to change
		...
		ret = ft8736_swipe_command(dev, SWIPE_DISABLE_CMD);
	} else {
		TOUCH_I("%s swipe mode = %s\n", __func__,  "Enable");
		...
		ret = ft8736_reg_write(dev, SWIPE_DIST_THRESHOLD_CTRL, &swipe_data, sizeof(swipe_data));
		...
		swipe_data_2[5] = 0; // Actually, No use high byte values.
		swipe_data_2[6] = ts->swipe.info.wrong_dir_thes;
		ret = ft8736_reg_write(dev, SWIPE_RATIO_THRESHOLD_CTRL, &swipe_data_2, sizeof(swipe_data_2));

		ret = ft8736_swipe_command(dev, SWIPE_ACTIVE_AREA_CMD);
	}

	return ret;
}

static int ft8736_tci_control(struct device *dev, int type)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
	...
	case TCI_CTRL_SET:
		...
		ret = ft8736_reg_write(dev, 0xF6, &data, 1);
		data = ts->tci.mode;
		ret = ft8736_reg_write(dev, 0xD0, &data, 1);
		break;

	case TCI_CTRL_CONFIG_COMMON:
		data = (((d->tci_debug_type) & 0x02) << 3) | ((d->tci_debug_type) & 0x01);
		ret = ft8736_reg_write(dev, 0xE5, &data, 1);	// Fail Reason Debug Function Enable

		data = 0x28;
		ret |= ft8736_reg_write(dev, 0xB4, &data, 1);	// Active Area LSB of X1
		data = 0x00;
		ret |= ft8736_reg_write(dev, 0xB5, &data, 1);	// Active Area MSB of X1 (40)
		data = 0xA8;
		ret |= ft8736_reg_write(dev, 0xB6, &data, 1);	// Active Area LSB of X2
		data = 0x02;
		ret |= ft8736_reg_write(dev, 0xB7, &data, 1);	// Active Area MSB of X2 (680)
		data = 0x22;
		ret |= ft8736_reg_write(dev, 0xB8, &data, 1);	// Active Area LSB of Y1
		data = 0x00;
		ret |= ft8736_reg_write(dev, 0xB9, &data, 1);	// Active Area MSB of Y1 (34)
		data = 0x7E;
		ret |= ft8736_reg_write(dev, 0xBA, &data, 1);	// Active Area LSB of Y2
		data = 0x05;
		ret |= ft8736_reg_write(dev, 0xBB, &data, 1);	// Active Area MSB of Y2 (1406)
		break;

	case TCI_CTRL_CONFIG_TCI_1:
		data = 0x0A;
		ret = ft8736_reg_write(dev, 0xBC, &data, 1);	// Touch Slop (10mm)
		data = 0x0A;
		ret |= ft8736_reg_write(dev, 0xC4, &data, 1);	// Touch Distance (10mm)
		data = 0x46; //0x32;
		ret |= ft8736_reg_write(dev, 0xC6, &data, 1);	// Time Gap Max (700ms)
		data = 0x02;
		ret |= ft8736_reg_write(dev, 0xCA, &data, 1);	// Total Tap Count (2)
		data = info1->intr_delay;
		ret |= ft8736_reg_write(dev, 0xCC, &data, 1);	// Interrupt Delay (700ms or 0ms)
		break;

	case TCI_CTRL_CONFIG_TCI_2:
		data = 0x0A;
		ret = ft8736_reg_write(dev, 0xBD, &data, 1);	// Touch Slop (10mm)
		data = 0xFF; //0xC8;
		ret |= ft8736_reg_write(dev, 0xC5, &data, 1);	// Touch Distance (?? 200mm ??)
		data = 0x46;
		ret |= ft8736_reg_write(dev, 0xC7, &data, 1);	// Time Gap Max (700ms)
		data = info2->tap_count;
		ret |= ft8736_reg_write(dev, 0xCB, &data, 1);	// Total Tap Count (2)
		data = 0x25;
		ret |= ft8736_reg_write(dev, 0xCD, &data, 1);	// Interrupt Delay (370ms ??)
		break;
...
static int ft8736_lpwg_control(struct device *dev, u8 mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct tci_info *info1 = &ts->tci.info[TCI_1];
	//struct ft8736_data *d = to_ft8736_data(dev);
	...
	case LPWG_DOUBLE_TAP:
		...
		ret = ft8736_tci_control(dev, TCI_CTRL_CONFIG_TCI_1);
		ret |= ft8736_tci_control(dev, TCI_CTRL_CONFIG_COMMON);
		ret |= ft8736_tci_control(dev, TCI_CTRL_SET);
	...
	case LPWG_PASSWORD:
		...
		ret = ft8736_tci_control(dev, TCI_CTRL_CONFIG_TCI_1);
		ret |= ft8736_tci_control(dev, TCI_CTRL_CONFIG_TCI_2);
		ret |= ft8736_tci_control(dev, TCI_CTRL_CONFIG_COMMON);
		ret |= ft8736_tci_control(dev, TCI_CTRL_SET);
...
//		ret = ft8736_tci_control(dev, TCI_CTRL_CONFIG_TCI_1);
	...
	TOUCH_I("ft8736_lpwg_control mode = %d\n", mode);

	return ret;
}


static int ft8736_deep_sleep(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	u8 data;

	TOUCH_I("ft8736_deep_sleep = %d\n", mode);
	...
	if(mode) {
		...
		return ft8736_reg_write(dev, 0xA5, &data, 1);
	}
	else {
...
#if 0
static void ft8736_debug_tci(struct device *dev)
{
	struct ft8736_data *d = to_ft8736_data(dev);
	...
	ft8736_reg_read(dev, TCI_DEBUG_R, &rdata, sizeof(rdata));
...
int ft8736_lpwg_reset(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);

	TOUCH_I("ft8736_lpwg_reset\n");
	touch_gpio_direction_output(ts->reset_pin, 0);
	touch_msleep(5);
	touch_gpio_direction_output(ts->reset_pin, 1);
...
static int ft8736_lpwg_mode(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
	u8 next_state;
...
#if 1
	TOUCH_I(
		"ft8736_lpwg_mode : mode[%d], screen[%s], sensor[%s], qcover[%s]\n",
		ts->lpwg.mode,
...
//	mfts_mode = touch_boot_mode_check(dev);
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		if ((ts->role.mfts_lpwg) && (d->state == TC_STATE_ACTIVE)) {
			ft8736_lpwg_control(dev, LPWG_DOUBLE_TAP);
			ft8736_swipe_control(dev);
	...
	if (d->state == TC_STATE_ACTIVE) {
		if (ts->lpwg.screen == 0) {
			if((ts->lpwg.mode == 0) && !ts->role.mfts_lpwg) {
				...
				ret = ft8736_deep_sleep(dev, 1);
				TOUCH_I("4 0\n");
			} else if (ts->lpwg.sensor == 1) {
				next_state = TC_STATE_LPWG;
				TOUCH_I("%s\n","STATE_LPWG");
				ret = ft8736_lpwg_control(dev, ts->lpwg.mode); // LCD SLP In or LCD Deep Sleep, 100ms -> Touch LPWG
				ret = ft8736_swipe_control(dev);
			}
			else {
				next_state = TC_STATE_DEEP_SLEEP;
				ret = ft8736_deep_sleep(dev, 1); // LCD Deep Sleep, 100ms -> Touch Deep Sleep, DSV Off
				TOUCH_I("4\n");
			}
		}
		else {
		...
		if (ts->lpwg.screen == 0) {
			...
			if(d->tci_debug_type & 0x02)
				ft8736_report_tci_fr_buffer(dev); // Report fr before touch IC reset
			touch_gpio_direction_output(ts->reset_pin, 0);
			TOUCH_I("%s\n","STATE_ACTIVE");
			touch_msleep(5);
			touch_gpio_direction_output(ts->reset_pin, 1);
//			touch_msleep(105); //  case resume version 0.0
		...
		if (ts->lpwg.screen == 0) {
			if(ts->lpwg.mode == 0){
				...
				touch_gpio_direction_output(ts->reset_pin, 1);
				touch_msleep(200);
				ret = ft8736_deep_sleep(dev, 0);
				ret = ft8736_lpwg_control(dev, ts->lpwg.mode);
				ret = ft8736_swipe_control(dev);
			}
			else {
				next_state = TC_STATE_DEEP_SLEEP; // Do nothing
...
static int ft8736_lpwg(struct device *dev, u32 code, void *param)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int *value = (int *)param;
...
int ft8736_lpwg_set(struct device *dev){
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
...
static void ft8736_connect(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
	...
	ft8736_reg_write(dev, SPR_CHARGER_STS, &d->charger, sizeof(u8));
}

#if 0
static void ft8736_lcd_event_read_reg(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	u32 rdata[5] = {0};

//	ft8736_xfer_msg_ready(dev, 5);
...
void ft8736_xfer_msg_ready(struct device *dev, u8 msg_cnt)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
...
static int ft8736_usb_status(struct device *dev, u32 mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
#if 0
static int ft8736_wireless_status(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int ft8736_earjack_status(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int ft8736_debug_tool(struct device *dev, u32 value)
{
	struct touch_core_data *ts = to_touch_core(dev);

	if (value == DEBUG_TOOL_ENABLE) {
//		ts->driver->irq_handler = ft8736_sic_abt_irq_handler;
	} else {
		ts->driver->irq_handler = ft8736_irq_handler;
	}

	return 0;
}
static int ft8736_debug_option(struct device *dev, u32 *data)
{
	u32 chg_mask = data[0];
	u32 enable = data[1];
	...
	case DEBUG_OPTION_1:
//		if (enable)	/* ASC */
//			ft8736_asc_control(dev, ASC_ON);
//		else
//			ft8736_asc_control(dev, ASC_OFF);
		break;
	case DEBUG_OPTION_2:
...
static void ft8736_fb_notify_work_func(struct work_struct *fb_notify_work)
{
	struct ft8736_data *d =
			container_of(to_delayed_work(fb_notify_work),
				struct ft8736_data, fb_notify_work);
	int ret = 0;
...
#endif
/*
static int ft8736_fb_notifier_callback(struct notifier_block *self,
...
static int ft8736_notify_charger(struct device *dev, ulong event, void *data)
{
...
static int ft8736_notify_normal(struct device *dev, ulong event, void *data)
{
	struct touch_core_data *ts = to_touch_core(dev);
//	struct ft8736_data *d = to_ft8736_data(dev);
	int ret = 0;
...
//		atomic_set(&d->watch.state.rtc_status, RTC_CLEAR);
	...
	case LCD_EVENT_LCD_MODE:
		...
		ft8736_lcd_mode(dev, *(u32 *)data);
		ret = ft8736_check_mode(dev);
		if (ret == 0)
	...
	case LCD_EVENT_READ_REG:
		...
		ft8736_lcd_event_read_reg(dev);
		break;
#endif
	case NOTIFY_CONNECTION:
		TOUCH_I("NOTIFY_CONNECTION!\n");
		ret = ft8736_usb_status(dev, *(u32 *)data);
//		if (ft8736_asc_usable(dev))	/* ASC */
//			ft8736_asc_toggle_delta_check(dev);
		break;
#if 0
	case NOTIFY_WIRELEES:
		TOUCH_I("NOTIFY_WIRELEES!\n");
		ret = ft8736_wireless_status(dev, *(u32 *)data);
//		if (ft8736_asc_usable(dev))	/* ASC */
//			ft8736_asc_toggle_delta_check(dev);
		break;
	case NOTIFY_EARJACK:
		TOUCH_I("NOTIFY_EARJACK!\n");
		ret = ft8736_earjack_status(dev, *(u32 *)data);
		break;
#endif
	case NOTIFY_IME_STATE:
		TOUCH_I("NOTIFY_IME_STATE!\n");
//		ret = ft8736_reg_write(dev, REG_IME_STATE,
//			(u32*)data, sizeof(u32));
		break;
#if 0
	case NOTIFY_DEBUG_TOOL:
		ret = ft8736_debug_tool(dev, *(u32 *)data);
		TOUCH_I("NOTIFY_DEBUG_TOOL!\n");
		break;
	case NOTIFY_CALL_STATE:
		TOUCH_I("NOTIFY_CALL_STATE!\n");
		ret = ft8736_reg_write(dev, REG_CALL_STATE,
			(u32 *)data, sizeof(u32));
//		if (ft8736_asc_usable(dev))	/* ASC */
//			ft8736_asc_toggle_delta_check(dev);
		break;
	case NOTIFY_DEBUG_OPTION:
		...
		ret = ft8736_debug_option(dev, (u32 *)data);
		break;
	case NOTIFY_ONHAND_STATE:
		TOUCH_I("NOTIFY_ONHAND_STATE!\n");
//		if (ft8736_asc_usable(dev)) {	/* ASC */
//			ft8736_asc_toggle_delta_check(dev);
//			ft8736_asc_write_onhand(dev, *(u32 *)data);
//		}
		break;
#endif
/*	case NOTIFY_QUICKCOVER_STATE:
		TOUCH_I("NOTIFY_QUICKCOVER_STATE!: %d\n", *(u8 *)data);
		ret = ft8736_reg_write(dev, SPR_QUICKCOVER_STS, (u8 *)data, sizeof(u8));
		break; */
	default:
...
static int ft8736_notify(struct device *dev, ulong event, void *data)
{
	TOUCH_TRACE();

	if (touch_boot_mode() == TOUCH_CHARGER_MODE) {
		TOUCH_I("CHARGER MODE notify\n");
		return ft8736_notify_charger(dev,event,data);
	}

	return ft8736_notify_normal(dev,event,data);
}

static void ft8736_init_locks(struct ft8736_data *d)
{
	mutex_init(&d->rw_lock);
	mutex_init(&d->fb_lock);
}

static int ft8736_probe(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = NULL;
	...
	if (!d) {
		TOUCH_E("failed to allocate ft8736 data\n");
		return -ENOMEM;
	}
	...
	if (touch_boot_mode() == TOUCH_CHARGER_MODE) {
		...
		/* Deep Sleep */
		touch_msleep(100); // ???????????????????????
		ft8736_deep_sleep(dev, 1);
		return 0;
	}

	ft8736_get_swipe_info(dev);
	ft8736_get_tci_info(dev);
...
static int ft8736_remove(struct device *dev)
{
	TOUCH_TRACE();
#ifdef FTS_APK_DEBUG
...
static int ft8736_fw_compare(struct device *dev, const struct firmware *fw)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
...
#if 1  // MH4_FT8736
		bin_fw_version = fw->data[0x1100+0x0e];
#endif
...
static int ft8736_fwboot_upgrade(struct device *dev, const struct firmware *fw_boot)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	TOUCH_I("%s -Firmware (Pramboot) download START\n", __func__);
	...
	if(fw_check_buf == NULL)
		return -ENOMEM;
#if 1 // MH4_FT8736
	ret = ft8736_reg_write(dev, 0x55, i2c_buf, 0);
	touch_msleep(2);
#endif
...
//---- Reset CTPM -----
		...
		touch_msleep(50);
		touch_gpio_direction_output(ts->reset_pin, 1);
		touch_msleep(i); // Variable delay.

//----- Set Upgrade Mode -----
#if 1 // MH4_FT8736
		i2c_buf[0] = 0xAA;
		ret = ft8736_reg_write(dev, 0x55, i2c_buf, 1);
#endif
		if(ret < 0) {
			TOUCH_E("i2c error\n");
			goto FAIL;
...
//----- Check ID -----
		...
		ret = ft8736_reg_read(dev, 0x90, i2c_buf, 2);
		if(ret < 0) {
			TOUCH_E("i2c error\n");
			goto FAIL;
		}
#if 1 // MH4_FT8736
		if(i2c_buf[0] == 0x87&& i2c_buf[1] == 0x36){
			touch_msleep(50);
		TOUCH_I(" Pramboot Check ID : 0x%x , 0x%x\n", i2c_buf[0], i2c_buf[1]);
			break;
		}
#endif
	}
...
//----- Write F/W (Pramboot) Binary to CTPM -----
	...
	for (i = 0; i < packet_num; i++) {
		...
		ret = ft8736_reg_write(dev, 0xAE, i2c_buf, packet_len + 5);
		if(ret < 0) {
			TOUCH_E("i2c error\n");
			goto FAIL;
		}
...
//----- Pramboot Write Verification ------

#if 0   // Verify method 1: Check sum
	ret = ft8736_reg_write(dev, 0xCC, i2c_buf, 1);
	...
	for (i = 0; i < packet_num; i++) {
		...
		ret = ft8736_cmd_read(dev, i2c_buf, 4, fw_check_buf+packet_addr, packet_len);
		if(ret < 0) {
			TOUCH_E("i2c error\n");
			goto FAIL;
		}
	...
	for (i = 0; i < fw_size; i++) {
		if(fw_check_buf[i] != fw_data[i]) {
			TOUCH_I("%s - Verify Failed %d %d %d!!\n", __func__,i, fw_check_buf[i], fw_data[i]);
			goto FAIL;
		}
	}
	TOUCH_I("%s -Pramboot write Verify OK !!\n", __func__);
...
// ----- Start App -----
	TOUCH_I("%s - Start App\n", __func__);
	ret = ft8736_reg_write(dev, 0x08, i2c_buf, 0);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		goto FAIL;
	}
	...
	if(fw_check_buf)
		kfree(fw_check_buf);
	TOUCH_I("== Firmware (Pramboot) download SUCCESS !!! ==\n");
	return 0;
...
static int ft8736_fw_upgrade(struct device *dev, const struct firmware *fw)
{
//	struct touch_core_data *ts = to_touch_core(dev);
...
//----- Check ID -----
		ret = ft8736_reg_read(dev, 0x90, i2c_buf, 2);
#if 1  // MH4_FT8736
		TOUCH_I("FW download Check ID [%d] : 0x%x , 0x%x\n", i, i2c_buf[0], i2c_buf[1]);
		if(i2c_buf[0] == 0x87 && i2c_buf[1] == 0xc6)    // 0x87, 0xC6
			break;
#endif
	}
...
//----- Change to write flash mode -----
	//- Flash : 05, Read Flashe type : 81:FT5003, 80: Winbond
	//- Clk speed : 00:48M 01:24M

	ret = ft8736_reg_read(dev, 0x05, i2c_buf, 1); // Flash Type Read
	reg_val_id = i2c_buf[0];

	i2c_buf[0] = reg_val_id;
	i2c_buf[1] = 0x00;
	//i2c_buf[1] = 0x01;
	ret = ft8736_reg_write(dev, 0x05, i2c_buf, 2);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		goto FAIL;
	}
...
// ----- Change to write flash set range -----
	...
	//- 0x0C : Lcd
	i2c_buf[0] = 0x0A;
	ret = ft8736_reg_write(dev, 0x09, i2c_buf, 1);
#endif
	touch_msleep(2);

// ----- Flash Erase -----
	TOUCH_I("%s - Flash Erase App and Panel Parameter Area\n", __func__);

	ret = ft8736_reg_write(dev, 0x61, i2c_buf, 0);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		goto FAIL;
	}
	...
	i = 0;
	do {
		ret = ft8736_reg_read(dev, 0x6A, i2c_buf, 2);

		if(i2c_buf[0] == 0xF0 && i2c_buf[1] == 0xAA)  // FLASH STATUS ERASE OK 0xF0AA
		{
			TOUCH_I("Erase Done : %d, [0x%02x] , [0x%02x]\n",i, i2c_buf[0], i2c_buf[1]);
			break;
		}
		i++;
		mdelay(10);
...
// ----- Write F/W (App) Binary to CTPM -----
	...
	for (i = 0; i < packet_num; i++) {
...
#endif
		ret = ft8736_reg_write(dev, 0xBF, i2c_buf, packet_len + 5);
		if(ret < 0) {
			TOUCH_E("i2c error\n");
			goto FAIL;
		}
		...
		do {
			...
			if((u32)(i + 0x1000) == (((u32)(i2c_buf[0]) << 8) | ((u32)(i2c_buf[1]))))  // FLASH STATUS WRITE OK 0X1000+ADDR/ PAKET LENGTH
			{
				if((i & 0x007F) == 0) {  // LGE CODE
...
// -----Read out Checksum -----
	...
	ret = ft8736_reg_write(dev, 0x64, i2c_buf, 0);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		goto FAIL;
	}
	...
	i2c_buf[1] = (u8)(((u32)(packet_addr) & 0x0000FF00) >> 8);
	i2c_buf[2] = (u8)((u32)(packet_addr) & 0x000000FF);
	if( fw_size> LEN_FLASH_ECC_MAX)
		packet_len = LEN_FLASH_ECC_MAX;
	else
		packet_len = fw_size;
	i2c_buf[3] = (u8)(((u32)(packet_len) & 0x0000FF00) >> 8);
	i2c_buf[4] = (u8)((u32)(packet_len) & 0x000000FF);
	ret = ft8736_reg_write(dev, 0x65, i2c_buf, 5);
	...
	do {
		...
		if(i2c_buf[0] == 0xF0 && i2c_buf[1] == 0x55)  //0xF055 FLASH STATUS ECC OK
		{
	...
	ret = ft8736_reg_read(dev, 0x66, i2c_buf, 1);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		goto FAIL;
	}
	...
	if(i2c_buf[0] != fw_ecc)
	{
		TOUCH_E("FW (All.bin) download checksum ERROR : Reg 0x66 [0x%x] , fw_ecc [0x%x]\n", i2c_buf[0], fw_ecc);
		goto FAIL;
	}
	TOUCH_I("FW (All.bin) download checksum OK : Reg 0x66 [0x%x] , fw_ecc [0x%x]\n", i2c_buf[0], fw_ecc);
	TOUCH_I("== FW (All.bin) download SUCCESS !!! ==\n");


// ----- SW Reset CMD -----
	ret = ft8736_reg_write(dev, 0x07, i2c_buf, 0);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		goto FAIL;
	}
...
FAIL:
	...
	// Reset Anyway
	ft8736_power(dev, POWER_OFF);
	ft8736_power(dev, POWER_ON);
...
static int ft8736_upgrade(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
#if defined(CONFIG_LGE_MODULE_DETECT)
		if (ft8736_panel_type == CSOT_INCELL_FT8736) {
			strncpy(fwpath, ts->def_fwpath[1], sizeof(fwpath) - 1);
			TOUCH_I("Get Touch Firmware for MH4 CS0T-FT8736\n");
			// Bring-up (Need to remove after bring-up)
			ts->force_fwup = 1;
		}
#else
		strncpy(fwpath, ts->def_fwpath[1], sizeof(fwpath) - 1);
	...
	if (ft8736_fw_compare(dev, fw)) {
		...
		ret = ft8736_fwboot_upgrade(dev, fw_boot);
		if(ret < 0) {
...
static int ft8736_suspend(struct device *dev)
{
//	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
	//int mfts_mode = 0;
...
static int ft8736_resume(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
//	int mfts_mode = 0;
...
static int ft8736_init(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
//	u32 data = 1;
...
/*
	if (atomic_read(&ts->state.core) == CORE_PROBE) {
		TOUCH_I("ft8736 fb_notif register\n");
		fb_unregister_client(&ts->fb_notif);
		ts->fb_notif.notifier_call = ft8736_fb_notifier_callback;
...
//		ft8736_sic_abt_init(dev);

	ret = ft8736_ic_info(dev);
	if (ret < 0) {
...
#if 0
	ret = ft8736_reg_write(dev, tc_device_ctl, &data, sizeof(data));
	if (ret)
		TOUCH_E("failed to write \'tc_device_ctrl\', ret:%d\n", ret);

	ret = ft8736_reg_write(dev, tc_interrupt_ctl, &data, sizeof(data));
	if (ret)
		TOUCH_E("failed to write \'tc_interrupt_ctrl\', ret:%d\n", ret);
#endif
	ret = ft8736_reg_write(dev, SPR_CHARGER_STS, &d->charger, sizeof(u8));
	if (ret)
		TOUCH_E("failed to write \'spr_charger_sts\', ret:%d\n", ret);

	ret = ft8736_reg_write(dev, SPR_QUICKCOVER_STS, &ts->lpwg.qcover, sizeof(u8));
	if (ret)
		TOUCH_E("failed to write SPR_QUICKCOVER_STS, ret:%d\n", ret);
#if 0
	data = atomic_read(&ts->state.ime);
	ret = ft8736_reg_write(dev, REG_IME_STATE, &data, sizeof(data));
	if (ret)
...
int ft8736_check_status(struct device *dev)
{
	struct ft8736_data *d = to_ft8736_data(dev);
	int ret = 0;
...
#if 0
int ft8736_debug_info(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
	struct ft8736_touch_debug *debug = &d->info.debug;
	int ret = 0;
	int i = 0;
...
static int ft8736_irq_abs_data(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
	struct ft8736_touch_data *data = d->info.data;
...
//	int ret = 0;
	...
	for (i = 0; i < FTS_MAX_POINTS; i++) {
		...
		if(event == FTS_TOUCH_DOWN || event == FTS_TOUCH_CONTACT) {
			...
			//tdata->width_minor = 0;
			//tdata->orientation = 0;
			tdata->width_major = (u16)data[i].width_major;
			tdata->width_minor = (u16)data[i].width_minor;
...
int ft8736_irq_abs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
	struct ft8736_touch_data *data = d->info.data;
	...
	ret = ft8736_reg_read(dev, 0x02, point_buf+2, POINT_READ_BUF-2);
	...
	return ft8736_irq_abs_data(dev);
}

int ft8736_irq_lpwg(struct device *dev, int tci)
{
	struct touch_core_data *ts = to_touch_core(dev);
	//struct ft8736_data *d = to_ft8736_data(dev);
	...
	ret = ft8736_reg_read(dev, 0xD3, tci_data_buf, 2);
	if (ret < 0) {
	...
	//0x01(0001) : Knock On	(>>1) -> 0 (TCI_1)
	//0x02(0010) : Knock Code	(>>1) -> 1 (TCI_2)
	//0x04(0100) : Swipe 		(>>1) -> 2 (SWIPE)

	// Validate tci data
	if (((tci_data_buf[0] >> 1) != tci)
		|| (tci_data_buf[1] > MAX_TAP_COUNT)) {
		TOUCH_I("TCI data is invalid !!!\n");
		TOUCH_I("Result %d[tci:%d], TapCount %d[Max:%d]\n",
			(tci_data_buf[0] >> 1), tci, tci_data_buf[1], MAX_TAP_COUNT);
		return -1;
	}

	tap_count = tci_data_buf[1];

	ret = ft8736_reg_read(dev, 0xD3, tci_data_buf, tap_count*4 + 2);
	if (ret < 0) {
		TOUCH_E("Fail to read tci data\n");
		return ret;
	}
	...
	if (d->info.wakeup_type == KNOCK_1) {
		...
		ft8736_debug_tci(dev);
		ft8736_debug_swipe(dev);
	} else {
...
int ft8736_irq_report_tci_fr(struct device *dev)
{
	//struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
	int ret = 0;
	...
	ret = ft8736_reg_read(dev, 0xE6, &data, 1);
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
...
int ft8736_report_tci_fr_buffer(struct device *dev)
{
	//struct touch_core_data *ts = to_touch_core(dev);
	struct ft8736_data *d = to_ft8736_data(dev);
	int ret = 0, i;
	...
	// Knock-on
	for (i = 0; i < 25; i++) {
		ret = ft8736_reg_read(dev, 0xE7, &tci_fr_buffer, sizeof(tci_fr_buffer));
		if (!ret) {
			break;
		}
		msleep(2);
	...
	for (i = 0; i < 25; i++) {
		ret = ft8736_reg_read(dev, 0xE9, &tci_fr_buffer, sizeof(tci_fr_buffer));
		if (!ret) {
			break;
		}
		msleep(2);
...
int ft8736_irq_handler(struct device *dev)
{
	//struct touch_core_data *ts = to_touch_core(dev);
	...
	ret = ft8736_reg_read(dev, 0x01, &int_status, 1);
	if (ret < 0)
		return ret;
	//ret = ft8736_irq_abs(dev);

	if (int_status == 0x01) { // Finger
		ret = ft8736_irq_abs(dev);
	}
	else if (int_status == 0x02) { // TCI_1
		ret = ft8736_irq_lpwg(dev, TCI_1);
	}
	else if (int_status == 0x03) { // TCI_2
		ret = ft8736_irq_lpwg(dev, TCI_2);
	}
	else if (int_status == 0x04) { // LPWG Fail Reason Report (RT)
		ret = ft8736_irq_report_tci_fr(dev);
	}
	...
	else if (int_status == 0x06) { // SWIPE
		TOUCH_I("SWIPE_UP\n");
		ret = ft8736_irq_lpwg(dev, SWIPE);
	}
				...
				const char *buf, size_t count)
{
	...
	if (!strcmp(command, "write")) {
		data = (u8)value;
		if (ft8736_reg_write(dev, reg_addr, &data, sizeof(u8)) < 0)
			TOUCH_E("reg addr 0x%x write fail\n", reg_addr);
		else
			TOUCH_I("reg[%x] = 0x%x\n", reg_addr, data);
	} else if (!strcmp(command, "read")) {
		if (ft8736_reg_read(dev, reg_addr, &data, sizeof(u8)) < 0)
...
static struct attribute *ft8736_attribute_list[] = {
...
static const struct attribute_group ft8736_attribute_group = {
	.attrs = ft8736_attribute_list,
};

static int ft8736_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	TOUCH_TRACE();

	ret = sysfs_create_group(&ts->kobj, &ft8736_attribute_group);
	if (ret < 0)
		TOUCH_E("ft8736 sysfs register failed\n");

//	ft8736_watch_register_sysfs(dev);
	ft8736_prd_register_sysfs(dev);
//	ft8736_asc_register_sysfs(dev);	/* ASC */
//	ft8736_sic_abt_register_sysfs(&ts->kobj);

	return 0;
}

static int ft8736_get_cmd_version(struct device *dev, char *buf)
{
	struct ft8736_data *d = to_ft8736_data(dev);
	int offset = 0;
	int ret = 0;
...
static int ft8736_get_cmd_atcmd_version(struct device *dev, char *buf)
{
	struct ft8736_data *d = to_ft8736_data(dev);
	int offset = 0;
	int ret = 0;
...
static int ft8736_set(struct device *dev, u32 cmd, void *input, void *output)
{
	TOUCH_TRACE();
...
static int ft8736_get(struct device *dev, u32 cmd, void *input, void *output)
{
	int ret = 0;
	...
	switch (cmd) {
	case CMD_VERSION:
		ret = ft8736_get_cmd_version(dev, (char *)output);
		break;

	case CMD_ATCMD_VERSION:
		ret = ft8736_get_cmd_atcmd_version(dev, (char *)output);
		break;
...
static int ft8736_shutdown(struct device *dev){
	TOUCH_TRACE();
	return 0;
}
...
static struct touch_driver touch_driver = {
	.probe = ft8736_probe,
	.remove = ft8736_remove,
	.suspend = ft8736_suspend,
	.shutdown = ft8736_shutdown,
	.resume = ft8736_resume,
	.init = ft8736_init,
	.irq_handler = ft8736_irq_handler,
	.power = ft8736_power,
	.upgrade = ft8736_upgrade,
	.lpwg = ft8736_lpwg,
	.notify = ft8736_notify,
	.register_sysfs = ft8736_register_sysfs,
	.set = ft8736_set,
	.get = ft8736_get,
};

#define MATCH_NAME			"focaltech,ft8736"
...
#if defined(CONFIG_LGE_MODULE_DETECT)
static bool ft8736_get_device_type(void)
{
	bool bdevice = false;
...
static int __init touch_device_init(void)
{
	...
	TOUCH_TRACE();
#if defined(CONFIG_LGE_MODULE_DETECT)
	if (!ft8736_get_device_type()) {
		TOUCH_I("%s, ft8736 returned\n", __func__);
		return 0;
	}
	TOUCH_I("This panel is FT8736\n");
#endif /* CONFIG_LGE_MODULE_DETECT */

	TOUCH_I("FT8736__[%s] touch_bus_device_init\n", __func__);
