...
// Write the total length of data expected to be written to Flash
	...
	packet_num = (fw_size + LEN_FLASH_ECC_MAX - 1) / LEN_FLASH_ECC_MAX;  // ©ø¡í¨¬I FLASH, LEN_FLASH_ECC_MAX ¢¥UA¡×¡¤I CHECKSUM A©ªCa.
	for (i = 0; i < packet_num; i++) {
...
#if defined(CONFIG_LGE_MODULE_DETECT)
static bool ft8006p_get_device_type(void)
{
	bool bdevice = false;
	...
	//case CSOT_INCELL_FT8006P:
	case BOE_INCELL_FT8006P:
		ft8006p_panel_type = panel_type;
		TOUCH_I("%s, BOE ft8006p, found!\n", __func__);
		bdevice = true;
		break;
	case CPT_INCELL_FT8006P:
		ft8006p_panel_type = panel_type;
		TOUCH_I("%s, CPT ft8006p, found!\n", __func__);
		bdevice = true;
		break;
	default:
		break;
...
#if defined(CONFIG_LGE_MODULE_DETECT)
	if (!ft8006p_get_device_type()) {
		TOUCH_I("%s, ft8006p returned\n", __func__);
		return 0;
	}
	TOUCH_I("This panel is FT8006P\n");
#endif /* CONFIG_LGE_MODULE_DETECT */
