...
// extern bool flag_knock_on_ctrl_during_panel_on;  // not bringup display.
#endif
...
static int synaptics_sleep_control(struct device *dev, u8 mode)
{
	...
	if (calling /* && !flag_knock_on_ctrl_during_panel_on */) { // not bringup display.
...
static int synaptics_probe(struct device *dev)
{
	...
	d->factory_boot = lge_get_factory_boot();
