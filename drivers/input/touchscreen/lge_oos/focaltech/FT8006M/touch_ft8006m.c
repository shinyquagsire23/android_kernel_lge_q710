...
//	u8 rdata = 0;
	...
	TOUCH_I("[%s(chip id:%x)] : v%d.%d\n", (chip_id == 0x80) ? "FT8006M" : "FT8006P",\
			chip_id, is_official, fw_version);
...
static int ft8006m_tci_control(struct device *dev, int type)
{
	...
	case TCI_CTRL_CONFIG_COMMON:
		...
		data = 0x28;
		if(ft8006m_panel_type != CV1_TCL_FT8006P)
			ret |= ft8006m_reg_write(dev, 0xB4, &data, 1);	// Active Area LSB of X1
		else
			ret |= ft8006m_reg_write(dev, 0x92, &data, 1);	// Active Area LSB of X1  // FT8006P 0x92
...
static int ft8006m_fw_compare(struct device *dev, const struct firmware *fw)
{
	...
	if(ft8006m_panel_type != CV1_TCL_FT8006P)
		...
		bin_fw_version = fw->data[0x2100+0x0e]; // CV1S : FT8006P
...
// Skip code
	...
	ret = ft8006m_reg_read(dev, 0x05, i2c_buf, 1); // Flash Type Read
	reg_val_id = i2c_buf[0];
...
//- 0x0C : Lcd
	...
	// Write the total length of data expected to be written to Flash
		...
		ret = ft8006m_reg_write(dev, 0x7A, i2c_buf, 3);
		//	TOUCH_I(" FW total length  [0x%02x] ,[0x%02x] , [0x%02x] , [0x%02x]\n", (u8)i2c_buf[0], (u8)i2c_buf[1], (u8)i2c_buf[2]);
	}
...
//	int i = 0;
	...
	if (ts->test_fwpath[0] && (ts->test_fwpath[0] != 't' && ts->test_fwpath[1] != 0)) {
		...
		else if (ft8006m_panel_type == CV1_TCL_FT8006M) { /* CV1_TCL_FT8006M */
			strncpy(fwpath, ts->def_fwpath[2], sizeof(fwpath) - 1);
			TOUCH_I("Get Touch Firmware for CV1 TCL\n");
		}
		else { /* CV1_TCL_FT8006P */
			strncpy(fwpath, ts->def_fwpath[4], sizeof(fwpath) - 1);
			TOUCH_I("Get Touch Firmware for CV1 TCL 8006P\n");
		}
#else
		strncpy(fwpath, ts->def_fwpath[1], sizeof(fwpath) - 1);
#endif /* CONFIG_LGE_MODULE_DETECT */
...
//			ret = ft8006m_fw_upgrade(dev, fw);
		if(ft8006m_panel_type != CV1_TCL_FT8006P) {
			TOUCH_I("fwpath_boot[%s]\n", ts->def_fwpath[0]);  // FT8006M PARAMBOOT.BIN
			ret = request_firmware(&fw_boot, ts->def_fwpath[0], dev);
		}
		else {
			TOUCH_I("fwpath_boot[%s]\n", ts->def_fwpath[3]);  // FT8006P PARAMBOOT.BIN
			ret = request_firmware(&fw_boot, ts->def_fwpath[3], dev);
		}
		if (ret < 0) {
			TOUCH_E("fail to request_firmware fwpath: %s (ret:%d)\n", ts->def_fwpath[0], ret);
			release_firmware(fw);
...
#if 0
#if defined(CONFIG_LGE_MODULE_DETECT)
static enum ft8006m_get_device_type(void)
{
...
static int __init touch_device_init(void)
{
	...
	TOUCH_TRACE();
#if defined(CONFIG_LGE_MODULE_DETECT)
	ft8006m_panel_type = lge_get_panel_type();
	TOUCH_I("%s [lge_get_panel_type] = [%d]\n", __func__, ft8006m_panel_type);

	switch (ft8006m_panel_type) {
	case CV1_TIANMA_FT8006M:
		TOUCH_I("%s, Tianma ft8006m, found!\n", __func__);
		break;
	case CV1_TCL_FT8006M:
		TOUCH_I("%s, TCL ft8006m, found!\n", __func__);
		break;
	case CV1_TCL_FT8006P:
		TOUCH_I("%s, TCL ft8006p, found!\n", __func__);
		break;
	default:
		TOUCH_I("%s, ft8006m/p not found.\n", __func__);
		return 0;
	}	
#endif /* CONFIG_LGE_MODULE_DETECT */
