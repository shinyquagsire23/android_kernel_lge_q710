...
static irqreturn_t fpc1145_irq_handler(int irq, void *handle)
{
	...
	** since this is interrupt context (other thread...) */
	...
	sysfs_notify(&fpc1145->input->dev.kobj, NULL, dev_attr_irq.attr.name);
...
//	rc = select_pin_ctl(fpc1145, "fpc1145_spi_active");
	...
	if (of_property_read_bool(dev->of_node, "fpc,enable-wakeup")) {
        ...
        fpc1145->input = input_allocate_device();
        if(!fpc1145->input) {
            dev_err(dev, "input_allocate_deivce failed.");
            goto exit;
        }

        fpc1145->input->name = "fingerprint";
        fpc1145->input->dev.init_name = "lge_fingerprint";

        input_set_drvdata(fpc1145->input, fpc1145);
        rc = input_register_device(fpc1145->input);
        if(rc) {
            dev_err(dev, "input_register_device failed.");
            input_free_device(fpc1145->input);
            goto exit;
        }
	...
	rc = sysfs_create_group(&fpc1145->input->dev.kobj, &attribute_group);
	if (rc) {
...
static int fpc1145_remove(struct spi_device *spi)
{
	...
	sysfs_remove_group(&fpc1145->input->dev.kobj, &attribute_group);
...
static int fpc1145_suspend(struct spi_device *spi, pm_message_t mesg)
{
	struct fpc1145_data *fpc1145 = dev_get_drvdata(&spi->dev);
	struct device *dev = &spi->dev;
	if (of_property_read_bool(dev->of_node, "fpc,enable-wakeup")) {
		...
		select_pin_ctl(fpc1145, "fpc1145_reset_reset");
		(void)vreg_setup(fpc1145, "vdd_io", false);
		disable_irq( gpio_to_irq( fpc1145->irq_gpio ));
#ifdef HW240_TEMP_WA
		devm_free_irq(dev, gpio_to_irq(fpc1145->irq_gpio), fpc1145);
#endif
	}
	return 0;
}

static int fpc1145_resume(struct spi_device *spi)
{
	struct fpc1145_data *fpc1145 = dev_get_drvdata(&spi->dev);
	...
	if (of_property_read_bool(dev->of_node, "fpc,enable-wakeup")) {
		...
		(void)vreg_setup(fpc1145, "vdd_io", true);
		hw_reset(fpc1145);
#ifdef HW240_TEMP_WA
		irqf = IRQF_TRIGGER_RISING | IRQF_ONESHOT;
		rc = devm_request_threaded_irq(dev, gpio_to_irq(fpc1145->irq_gpio),
		...
		dev_dbg(dev, "requested irq %d\n", gpio_to_irq(fpc1145->irq_gpio));
#else
		enable_irq( gpio_to_irq( fpc1145->irq_gpio ));
#endif
	}
	return 0;
}
