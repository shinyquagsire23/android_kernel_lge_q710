...
#if !defined(CONFIG_SND_SOC_MAX98927) && !defined(CONFIG_SND_SOC_MAX98925) && !defined(CONFIG_SND_SOC_TAS2560)
	{
						...
						struct device *dev)
{
	...
	if (of_property_read_bool(dev->of_node,
				"ti,tas2560")) {
		dev_err(dev, "%s(): tas2560 audio support present\n",
				__func__);
        ...
        dev_err(dev, "%s(): tas2560 audio support not present\n",
                __func__);
#else
	if (of_property_read_bool(dev->of_node,
