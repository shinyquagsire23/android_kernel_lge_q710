...
static const struct epack_amp_cb amp_cb = {
	.get_amp_current_status = max98925_get_spk_amp_status,
	.get_amp_init_status = max98925_get_init_status,
	.amp_enable = max98925_spk_enable,
};
#endif
...
#ifdef CONFIG_LGE_EXTERNAL_SPEAKER
	ret = epack_amp_init(&amp_cb);
	if (ret)
		dev_err(codec->dev, "%s: epack initialization failed\n", __func__);
#endif
